...
# This version was created in June 2018 and adapted in september 2020.
# In this version of the script, only test moments T1 and T2 are included, not T3 (which refers to a different task).
# Only the 'intentional' and 'incidental' learner groups are kept; the 'unaware' group is too small (n=6) to integrate in the analysis.
# Further, only control items are included in this analysis.
...
# Remove observations for critical items
data <- data[data$verbtype!="critical",]
...
## Test for ceiling effects: one sample t-test against 100

# Is the mean score for control items significantly different from 100?
t.test(data$bin_score, mu = 1)
...
# We did not perform model explorations for this model.
# Instead, we used the same model structure that was also used for the main model with the critical items.

# Thus, we used the following model:
model_controls <- glmer(bin_score ~ input*testmoment*learningtype + (1+input|item) + (1|participant), family = 'binomial', data = data, control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e5))); toc()


## Results: obtain the summary of the final control items model

# Final model
final <- model_controls
summary(final)
...
# Relevelling: Final model with 'incidental' as intercept (the relevelled output can be used to double-check the results)
...
# Out of curiosity, we carried out the same model explorations as for the critical items.
# As for the random-effects structure, none of the random effects were found to improve model fit significantly.

## Explore contribution of random intercepts over items and over participants
...
anova(start_min_item_intercepts, start) # Start model is significantly better (p < .001); AIC difference of +- 400
...
anova(start_min_part_intercepts, start) # Start model is significantly better (p < .001); AIC difference of +- 30


## Explore different random-effects structures following Bates et al. (2015)

## Does the addition of extra random effects improve model fit? (main effects)
...
anova(start, input_part) # No significant improvement, and even slightly higher AIC. >> DISCARD
...
anova(start, input_item) # No significant improvement (p = .07); minimal AIC difference (1). >> DISCARD
# # # This differs from the critical items model.
...
anova(start, input_testmoment_part) # No significant improvement; AIC increase. >> DISCARD
...
anova(start, input_testmoment_item) # No significant improvement; AIC increase. >> DISCARD
...
anova(start, learningtype_input_item) # No significant improvement. >> DISCARD
...
anova(start, learningtype_testmoment_item) # No significant improvement. >> DISCARD
...
anova(start, input_learningtype_testmoment_item) # No significant improvement. >> DISCARD
